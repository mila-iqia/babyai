...
for seed in range(1, 6):
    for level, options in components.items():
        for use_cnn in [False, True]:
            subprocess.Popen("sbatch --account=rpp-bengioy --time=3:0:0 --ntasks=4 --gres=gpu:1 --mem=4G ./train_rl.sh python -m scripts.train_rl --env BabyAI-{}-v0 --algo ppo {} {} {} --tb --seed {} --save-interval 10".format(
                level,
