...
class BotAdvisor(Bot):
    ...
    def take_action_iterate(self, action):
        ...
        if subgoal == 'Open':
            ...
            if action == actions.toggle():
                self.stack.pop()
            # The only thing you can do in front of an unopened door that would change your state is left/right
        ...
        if subgoal == 'Pickup':
            ...
            # these are the only actions that can change your state (you are carrying nothing, and there is something pickup-able in front of you !)
                ...
                # basically the agent opens the box that he had to pickup, FOOL
        ...
        def drop_or_pickup_or_open_something_while_exploring():
            if action == actions.drop and carrying != new_carrying:
                # get that thing back
                new_fwd_cell = self.mission.grid.get(*new_fwd_pos)
                assert new_fwd_cell.type in ('key', 'box', 'ball')
                # Hopefully the bot would pickup THIS object and not something similar to it
                self.stack.append(('Pickup', ObjDesc(new_fwd_cell.type, new_fwd_cell.color)))
                return True
                ...
                self.stack.append(('Drop', None))
                self.stack.append(('GoToAdjPos', ObjDesc(fwd_cell.type, fwd_cell.color)))
                return True
        ...
        if subgoal == 'GoToObj':
            ...
            if obj_pos:
                ...
                if action in (actions.forward, actions.left, actions.right) and np.array_equal(obj_pos, new_fwd_pos):
                    # stack will be popped at next step if one of these actions accomplishes the task
                    return True
                elif action in (actions.drop, actions.pickup, actions.open):
                    if drop_or_pickup_or_open_something_while_exploring():
                        return True
                else:
                    # TODO: something doesn't seem right in this else block
                    path, _ = self.shortest_path(
                        lambda pos, cell: pos == obj_pos
                    )
