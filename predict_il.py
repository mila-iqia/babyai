...
class Model(nn.Module):
    def __init__(self, num_actions):
        ...
        # TODO: use one-hot action encoding?

        # GRU embedding + action to next observation
    ...
    def predict_obs(self, img, action, memory):
        ...
        # TODO: convert action into one-hot encoding?
        emb = torch.cat([x, action], dim=1)

        next_obs = self.next_obs(emb)

        return next_obs, memory
...
for batch_idx in range(50000):
    ...
    for step_idx in range(max_demo_len-1):
        ...
        y, memory = model.predict_obs(obs_step, act_step, memory)
